...
thetaSquaredSum = sum(sum(Theta1(:,2:end) .* Theta1(:,2:end))) + sum(sum(Theta2(:,2:end) .* Theta2(:,2:end)));
yMatrix = zeros(size(a3));
for i=1:(size(y,1)),
    yMatrix(i,y(i,1)) = 1;
end;
...
Theta2_grad = delta3 * a3;
% Theta2_grad = Theta2_grad(2:end,:);
Theta1_grad = delta2' * a2(:,2:end);
